...
def getCode():
    ...
    codeInput = TextInput(text ="Input code here", multiline = False)
    content.add_widget(codeInput)
    submitButton = Button(text = "Submit")
    content.add_widget(submitButton)
    code = ""
    codeCheck = True
    popup1 = Popup(title="Please enter your code", content = content, size_hint = (.4,.4))
    def logout(self):
        nonlocal code
        code = codeInput.text
        popup1.dismiss()
        nonlocal codeCheck
        codeCheck = False
    submitButton.bind(on_press= logout)
    popup1.open()
    while codeCheck:
        time.sleep(1)
